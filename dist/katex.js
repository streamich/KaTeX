...
},{"./src/ParseError":9,"./src/Settings":11,"./src/buildTree":16,"./src/parseTree":25,"./src/utils":28}],2:[function(require,module,exports){
...
},{"./MacroExpander":7,"./ParseError":9,"./environments":19,"./functions":22,"./parseData":24,"./symbols":26,"./unicodeRegexes":27,"./utils":28}],11:[function(require,module,exports){
"use strict";
...
},{"./fontMetrics.js":20}],13:[function(require,module,exports){
"use strict";
...
},{"./domTree":18,"./fontMetrics":20,"./symbols":26,"./utils":28}],14:[function(require,module,exports){
"use strict";
 ...
 */
var shouldHandleSupSub = function shouldHandleSupSub(group, options, isSup) {
    if (!group) {
        return false;
...
groupTypes.accent = function (group, options) {
    ...
    if (!group.value.isStretchy) {
        ...
        var _accentBody = makeSpan(["accent-body", vecClass], [makeSpan([], [accent])]);

        _accentBody = buildCommon.makeVList([{ type: "elem", elem: body }, { type: "kern", size: -clearance }, { type: "elem", elem: _accentBody }], "firstBaseline", null, options);
        ...
        _accentBody.children[1].style.marginLeft = 2 * skew + "em";
    } else {
        ...
        accentBody = buildCommon.makeVList([{ type: "elem", elem: body }, { type: "kern", size: 2 * ruleWidth }, { type: "elem", elem: accentBody }, { type: "kern", size: ruleWidth }], "firstBaseline", null, options);
    }
...
groupTypes.horizBrace = function (group, options) {
    ...
    var haveNote = group.type === "supsub";

    var newOptions = void 0;
    var note = void 0;
    var noteReset = void 0;

    if (haveNote) {
        if (group.value.sup) {
            newOptions = options.withStyle(style.fracNum());
            note = buildGroup(group.value.sup, newOptions);
            noteReset = makeSpan([style.reset(), style.fracNum().cls()], [note], newOptions);
        } else {
            newOptions = options.withStyle(style.fracDen());
            note = buildGroup(group.value.sub, newOptions);
            noteReset = makeSpan([style.reset(), style.fracDen().cls()], [note], newOptions);
        }
        group = group.value.base;
    }

    // Build the base group
    var body = buildGroup(group.value.base, options.withStyle(style.cramp()));

    // Create the stretchy element
    var braceBody = svgSpan(group, options);

    // Generate the vlist, with the appropriate kerns
    var braceShift = group.value.isOver ? -body.height - 2 * ruleWidth : body.depth + braceBody.height + 2 * ruleWidth;

    var vlist = buildCommon.makeVList([{ type: "elem", elem: body, shift: 0 }, { type: "elem", elem: braceBody, shift: braceShift }], "individualShift", null, options);

    if (haveNote) {
        var noteShift = braceShift + (group.value.isOver ? -braceBody.height - 3 * ruleWidth : noteReset.height + 3 * ruleWidth);

        var vSpan = makeSpan(["mord", group.value.isOver ? "mover" : "munder"], [vlist], options);

        vlist = buildCommon.makeVList([{ type: "elem", elem: vSpan, shift: 0 }, { type: "elem", elem: noteReset, shift: noteShift }], "individualShift", null, options);
    }
...
groupTypes.accentunder = function (group, options) {
    ...
    var accentBody = svgSpan(group, options);
    var accentShift = innerGroup.depth + 2 * ruleWidth + accentBody.height;

    // Generate the vlist, with the appropriate kerns
    var vlist = buildCommon.makeVList([{ type: "elem", elem: innerGroup, shift: 0 }, { type: "elem", elem: accentBody, shift: accentShift }], "individualShift", innerGroup.height, options);
...
groupTypes.strikeThru = function (group, options) {
    ...
    if (group.value.label === "\\sout") {
        style = options.style;
        line = makeSpan([style.reset(), Style.TEXT.cls(), "stretchy", "sout"]);
        line.height = fontMetrics.metrics.defaultRuleThickness / style.sizeMultiplier;
        line.maxFontSize = 1.0;

        lineShift = -0.5 * options.style.metrics.xHeight;
        vlist = buildCommon.makeVList([{ type: "elem", elem: innerGroup, shift: 0 }, { type: "elem", elem: line, shift: lineShift }], "individualShift", null, options);
        ...
        strut.style.height = innerGroup.depth + innerGroup.height + 0.4 + "em";
        ...
        var strikeThru = void 0;

        if (options.color) {
            strikeThru = makeSpan(["strike"], [], options);
            strikeThru.style.backgroundImage = inlineSVG(label, options.color);
        } else {
            strikeThru = makeSpan(["strike", label], [], options);
        }
        strikeThru.style.height = innerGroup.depth + innerGroup.height + 0.4 + "em";
        ...
        return makeSpan(["mord"], [new domTree.symbolNode("\u200B"), node], options);
    }
};
...
groupTypes.boxed = function (group, options) {
    var style = options.style;
    var innerGroup = buildGroup(group.value.body, options);
    var node = makeSpan([], [innerGroup], options);

    // Create a strut with which to enforce a span height.
    var strut = makeSpan([style.reset(), "strut"], [], options);
    ...
    var barrier = new domTree.symbolNode("\u200B");
...
groupTypes.xArrow = function (group, options) {
    ...
    // Build the inner group.
    var upperGroup = buildGroup(group.value.body, options);

    var lowerGroup = void 0;
    var lowerShift = void 0;
    var vlist = void 0;
    ...
    var arrowBody = svgSpan(group, options);

    var arrowShift = -style.metrics.axisHeight + arrowBody.depth;
    var upperShift = -style.metrics.axisHeight - arrowBody.height - 3 * ruleWidth;

    // Generate the vlist
    if (group.value.below) {
        lowerShift = -style.metrics.axisHeight + lowerGroup.height + arrowBody.height + 3 * ruleWidth;
        vlist = buildCommon.makeVList([{ type: "elem", elem: upperGroup, shift: upperShift }, { type: "elem", elem: arrowBody, shift: arrowShift }, { type: "elem", elem: lowerGroup, shift: lowerShift }], "individualShift", null, options);
    } else {
        vlist = buildCommon.makeVList([{ type: "elem", elem: upperGroup, shift: upperShift }, { type: "elem", elem: arrowBody, shift: arrowShift }], "individualShift", null, options);
    }
...
var svgSpan = function svgSpan(group, options) {
    ...
    if (!document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#BasicStructure", "1.1")) {
        ...
        imgObj = { // height, depth, className
            ...
            xLeftrightarrow: [0.275, 0.275, "x-arrow", "double-left-right-arrow"],
    ...
    var node = makeSpan(classArray, [], options);
    node.height = height;
    node.depth = depth;
    node.maxFontSize = 1.0;
    return node;
...
var inlineSVG = function inlineSVG(label, color) {
    if (label === "cancel") {
        return "url('data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27" + " viewBox=%270 0 30 30%27 preserveAspectRatio=" + "%27none%27%3E%3Cline" + " vector-effect=%27non-scaling-stroke%27 " + "x1=%270%27 y1=%2730%27" + " x2=%2730%27 y2=%270%27 stroke=%27" + color + "%27 stroke-width=%271.0%27/%3E%3C/svg%3E')";
    } else if (label === "bcancel") {
        return "url('data:image/svg+xml,%3Csvg xmlns=%27" + "http://www.w3.org/2000/svg%27" + " viewBox=%270 0 30 30%27 " + "preserveAspectRatio=%27none%27%3E%3Cline" + " vector-effect=%27non-scaling-stroke%27 x1=%270%27 y1=%270%27" + " x2=%2730%27 y2=%2730%27 stroke=%27" + color + "%27 stroke-width=%271.0%27/%3E%3C/svg%3E')";
    } else {
        // xcancel
        return "url('data:image/svg+xml,%3Csvg " + "xmlns=%27http://www.w3.org/2000/svg%27" + " viewBox=%270 0 30 30%27 " + "preserveAspectRatio=%27none%27%3E%3Cline" + " vector-effect=%27non-scaling-stroke%27 x1=%270%27 y1=%270%27" + " x2=%2730%27 y2=%2730%27 stroke=%27" + color + "%27 stroke-width=%271.0%27/%3E%3Cline" + " vector-effect=%27non-scaling-stroke%27 x1=%270%27 y1=%2730%27" + " x2=%2730%27 y2=%270%27 stroke=%27" + color + " %27 stroke-width=%271.0%27/%3E%3C/svg%3E')";
    }
};

var customBracket = function customBracket(group, options, lineWt) {
    // Create an over|under bracket with a custom line weight and height.
    var bracketBody = makeSpan(["stretchy", "custom-bracket"], [], options);
    ...
    var height = 300;
    lineWt = Math.round(calculateSize(group.value.thickness.value, options.style) * 1000);
    ...
    var step = height - lineWt;

    if (group.value.label === "\\overbracket") {
        bracketBody.style.backgroundImage = "url('data:image/" + "svg+xml,%3Csvg" + " xmlns=%27http://www.w3.org/2000/svg%27%3E%3Csvg " + "width=%2750.1%25%27" + " viewBox=%270 0 400000 " + height + "%27 preserveAspectRatio=%27xMinYMin" + " slice%27%3E%3Cpath fill=%27black%27 d=%27M0 0 h400000 v" + lineWt + " L" + lineWt + " " + lineWt + " v" + step + " h-" + lineWt + "Z%27/%3E%3C/svg%3E%3Csvg x=%2750%25%27 " + "width=%2750%25%27" + " viewBox=%270 0 400000 " + height + "%27 preserveAspectRatio=%27xMaxYMin" + " slice%27%3E%3Cpath fill=%27black%27 d=%27M0 0 h400000 v" + height + " h-" + lineWt + " v-" + step + " L0 " + lineWt + "Z%27/%3E%3C/svg%3E%3C/svg%3E')";
    } else {
        bracketBody.style.backgroundImage = "url('data:image/svg+xml,%3Csvg" + " xmlns=%27http://www.w3.org/2000/" + "svg%27%3E%3Csvg width=%2750.1%25%27" + " viewBox=%270 0 400000 " + height + "%27 preserveAspectRatio=%27xMinYMin" + " slice%27%3E%3Cpath fill=%27black%27 d=%27M0 0 h" + lineWt + " v" + step + " L400000 " + step + " v" + lineWt + " L0 " + height + "Z%27/%3E%3C/svg%3E%3Csvg x=%2750%25%27 width=%2750%25%27" + " viewBox=%270 0 400000 " + height + "%27 preserveAspectRatio=%27xMaxYMin" + " slice%27%3E%3Cpath fill=%27black%27 d=%27M0 0 v" + height + " h400000 v-" + height + " h-" + lineWt + " v" + step + "Z%27/%3E%3C/svg%3E%3C/svg%3E')";
    }
    return bracketBody;
};
...
},{"./ParseError":9,"./Style":12,"./buildCommon":13,"./delimiter":17,"./domTree":18,"./fontMetrics":20,"./utils":28,"babel-runtime/core-js/json/stringify":2}],15:[function(require,module,exports){
"use strict";
...
var mathNodeFromLabel = function mathNodeFromLabel(label) {
    ...
    var node = new mathMLTree.MathNode("mo", [new mathMLTree.TextNode(codePoint[label.substr(1)])]);
...
groupTypes.accentunder = function (group, options) {
    var accentNode = mathNodeFromLabel(group.value.label);
    var node = new mathMLTree.MathNode("munder", [buildGroup(group.value.body, options), accentNode]);
    node.setAttribute("accentunder", "true");
    return node;
};

groupTypes.strikeThru = function (group, options) {
    var node = new mathMLTree.MathNode("memclose", [buildGroup(group.value.body, options)]);
    var notation = void 0;
...
groupTypes.boxed = function (group, options) {
    var inner = buildExpression(group.value.value, options);
    var node = new mathMLTree.MathNode("menclose", inner);
...
groupTypes.xArrow = function (group, options) {
    var arrowNode = mathNodeFromLabel(group.value.label);
    var upperNode = buildGroup(group.value.body, options);
    var node = void 0;

    if (group.value.below) {
        var lowerNode = buildGroup(group.value.below, options);
...
},{"./ParseError":9,"./buildCommon":13,"./fontMetrics":20,"./mathMLTree":23,"./symbols":26,"./utils":28}],16:[function(require,module,exports){
"use strict";
...
},{"./ParseError":9,"./Style":12,"./buildCommon":13,"./fontMetrics":20,"./symbols":26,"./utils":28}],18:[function(require,module,exports){
"use strict";
...
},{"./unicodeRegexes":27,"./utils":28}],19:[function(require,module,exports){
"use strict";
...
},{"./ParseError":9,"./Style":12,"./parseData":24}],20:[function(require,module,exports){
"use strict";
...
},{"./Style":12,"./fontMetricsData":21,"./unicodeRegexes":27}],21:[function(require,module,exports){
"use strict";
...
defineFunction(["\\acute", "\\grave", "\\ddot", "\\tilde", "\\bar", "\\breve", "\\check", "\\hat", "\\vec", "\\dot", "\\widehat", "\\widetilde", "\\overrightarrow", "\\overleftarrow", "\\Overrightarrow", "\\overleftrightarrow", "\\overgroup", "\\overlinesegment", "\\overleftharpoon", "\\overrightharpoon"], {
    ...
    var isStretchy = !utils.contains(["\\acute", "\\grave", "\\ddot", "\\tilde", "\\bar", "\\breve", "\\check", "\\hat", "\\vec", "\\dot"], context.funcName);

    var isShifty = !isStretchy || utils.contains(["\\widehat", "\\widetilde", "\\overrightarrow", "\\overrightharpoon"], context.funcName);
...
defineFunction(["\\overbrace", "\\overbracket", "\\underbrace", "\\underbracket"], {
    numArgs: 1,
    numOptionalArgs: 2,
    argTypes: ["size", "size", "original"]
}, function (context, args) {
    var thickness = args[0];
    var height = args[1];
    var base = args[2];
    return {
        type: "horizBrace",
...
},{"./ParseError":9,"./parseData":24,"./utils":28}],23:[function(require,module,exports){
"use strict";
